...
class BalmerLines:
    ...
    def plot_integrated_spectra(self, extinction_correction=True):
        ...
        integral_h_alpha = simps(spectra_h_alpha, wavelengths)
        integral_h_beta = simps(spectra_h_beta, wavelengths)
    ...
    def plot_line_ratio(self):
        """
        This function plots the H-alpha/H-beta line ratio with and without dust correction.
        """
        
        ha_intensity_with = self.intensities_ha_with
        ha_intensity_without = self.intensities_ha_without
        hb_intensity_with = self.intensities_hb_with
        hb_intensity_without = self.intensities_hb_without

        ratio_with = ha_intensity_with / hb_intensity_with
        ratio_without = ha_intensity_without / hb_intensity_without

        plt.plot(self.velocities, ratio_with, label='H-alpha/H-beta Ratio with Dust Correction')
        plt.plot(self.velocities, ratio_without, label='H-alpha/H-beta Ratio without Dust Correction')
        plt.title('H-alpha to H-beta Line Ratio')
        plt.xlabel('Input velocity (km/s)')
